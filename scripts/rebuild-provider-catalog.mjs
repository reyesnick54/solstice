...
const FX_REFERENCE_CATALOG_ENTRIES = await loadTsExport(
  './packages/payments/src/fx-reference/catalog-entries.ts',
  'FX_REFERENCE_CATALOG_ENTRIES',
);
const FX_REFERENCE_BLOCKED_CATALOG_ENTRY = await loadTsExport(
  './packages/payments/src/fx-reference/catalog-entries.ts',
...
function addEntries(entries) {
  for (const entry of entries ?? []) {
    if (entry?.provider_id) {
      byId.set(entry.provider_id, normalizeProviderEntry(entry));
    }
  }
}
...
const catalog = {
  ...
  notes:
    `Catalog rebuild: ${providerCount} unique providers merged from Waves 2–6 implementations. ` +
