...
- Chunk 34R implements the local development SunRey node at
  `packages/sunrey-chain/rust`. Capability `sunrey-local-node` is
  `IMPLEMENTED`. Production BFT, public network, and MoonRey issuance
  are not implemented. See `docs/architecture/chunk-34-resume.md`.
  The earlier documentation-only gate is historical:
