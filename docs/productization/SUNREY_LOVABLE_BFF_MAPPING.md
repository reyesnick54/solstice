...
| INTERNAL ALPHA QUOTE | `/api/v1/exchange/quotes` | POST | required | `sunrey.consumer.exchange.quote.v1` | DigitalAssetLifecycle + market maker book | LIVE_ALPHA | accepts `quantity` or `spendMinorUnits`; quote currency is `SANDBOX_USD` |
| INTERNAL ALPHA ORDER | `/api/v1/exchange/orders` | POST | required + owner + `confirmed` + step-up | `sunrey.consumer.exchange.order.v1` | matching + native clearing + Alpha chain | LIVE_ALPHA | returns `tradeIds`, `settlementIds`, `chainTransactionIds`; server-derived only |
| PORTFOLIO / TX / ECONOMY | `/api/v1/exchange/portfolio` `/transactions` `/economy` `/economy/status` | GET | required | portfolio / chain tx / economy | lifecycle + clearing | LIVE_ALPHA | `/portfolio` aliases holdings; nested economy mirrors top-level economy |
| SRC / MRC WALLETS | `/api/v1/wallets/SRC` `/api/v1/wallets/MRC` | GET | required | asset wallet aggregate | native clearing positions | LIVE_ALPHA | balances are server-derived; not frontend-supplied |
