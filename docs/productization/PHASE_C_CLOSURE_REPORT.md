...
PostgreSQL snapshot `V032__treasury_financial_control.sql`. Customer
migrations stay strictly increasing after Phase B/C collisions:
`V029__consumer_authentication.sql`, `V030__platform_api.sql`,
`V031__cards_productization.sql`, then treasury. Durable fixture
snapshots fail closed on corruption. Not a second ledger.
...
- PostgreSQL `db:migrate` / `test:persistence`: not executed here (no
  ...
  `V032__treasury_financial_control.sql` and
