...
| #18 | `feat/phase-9-pyramid-exchange-simulation` | Historical PYRAMID EXCHANGE simulation. The reserved future context is now `REYN_EXCHANGE` at `packages/reyn-exchange` and is not implemented. |
| #19 | `cursor/phase-8-pyramid-economy-8977` | Historical PYR / Pyramid data-exchange work. Must not fork the ledger or Evidence Vault. Canonical coin reservation is now `REYN_COIN` at `packages/reyn-coin` and is not implemented. Public ticker remains UNDECIDED. |
