...
| #18 | `feat/phase-9-pyramid-exchange-simulation` | Historical PYRAMID EXCHANGE simulation. The reserved future context is now `SUNREY_EXCHANGE` at `packages/sunrey-exchange` and is not implemented. |
| #19 | `cursor/phase-8-pyramid-economy-8977` | Historical PYR / Pyramid data-exchange work. Must not fork the ledger or Evidence Vault. Canonical coin reservation is now `SUNREY_COIN` at `packages/sunrey-coin` and is not implemented. Public ticker remains TBD. |
