...
describe('consumer BFF payments SDK', () => {
  ...
  it('reads Grow My Money portfolio views without execution methods', async () => {
    ...
    assert.equal((portfolio as { frontendMathAuthoritative: boolean }).frontendMathAuthoritative, false);
    assert.equal((portfolio as { liveState: boolean }).liveState, false);
