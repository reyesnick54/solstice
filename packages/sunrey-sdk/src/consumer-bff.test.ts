...
describe('consumer BFF HIN contribution SDK', () => {
  it('calls unique HIN contribution routes', async () => {
    const urls: string[] = [];
    const client = createSunReyConsumerBffClient({
      baseUrl: 'http://example.test',
      getAccessToken: () => 'sandbox.basic_verified',
      generateRequestId: () => 'req_hin',
      fetchImpl: async (input, init) => {
        const url = typeof input === 'string' ? input : String(input);
        urls.push(`${init?.method ?? 'GET'} ${url}`);
        return new Response(JSON.stringify({ items: [], isMintFormula: false }), {
          status: 200,
          headers: { 'content-type': 'application/json' },
        });
      },
    });
    await client.listHinContributions();
    await client.getHinMetrics();
    await client.getHinMySummary();
    await client.listHinValuationMethodologies();
    assert.ok(urls.some((row) => row.includes('/api/v1/hin/contributions')));
    assert.ok(urls.some((row) => row.includes('/api/v1/hin/metrics')));
    assert.ok(urls.some((row) => row.includes('/api/v1/hin/me/summary')));
    assert.ok(urls.some((row) => row.includes('/api/v1/hin/valuation-methodologies')));
  });
});
