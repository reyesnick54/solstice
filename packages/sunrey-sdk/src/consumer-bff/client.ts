...
export class SunReyConsumerBffClient {
  ...
  async createWithdrawalQuote(
    input: { readonly assetId?: string; readonly quantity: string; readonly destination?: string },
    options?: BffRequestOptions,
  ): Promise<Record<string, unknown>> {
  ...
  async createExchangeProposal(
    input: { readonly side: 'BUY' | 'SELL'; readonly quantity: string; readonly origin?: 'HUMAN' | 'AGENT' },
    options?: BffRequestOptions,
  ): Promise<Record<string, unknown>> {
