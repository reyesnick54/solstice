...
export const ACCESS_INVARIANT_STATEMENTS: Readonly<Record<AccessEconomyInvariantId, string>> = Object.freeze({
  ...
  NO_SOCIAL_CREDIT_SCORING: 'No social-credit or reputation score influences allocation.',
  ...
  NO_ACCESS_RIGHT_IMPLIES_OWNERSHIP: 'An access right never conveys title or ownership.',
  NO_CAPACITY_CREATED_BY_QUERY: 'Published capacity is derived from productive output, not from request volume.',
  NO_PROVIDER_SELF_REPORT_TRUSTED_WHERE_INDEPENDENT_PROOF_REQUIRED:
    'Stale or unavailable capacity evidence fails closed instead of trusting provider self-report.',
  ...
  BFF_CANNOT_INVENT_CAPACITY_OR_PRICING:
    'When canonical Exchange pricing is unavailable, no fallback price or invented capacity is granted.',
  ENTITLEMENT_IS_NOT_TRANSFERABLE_MONEY:
    'Entitlements remain non-transferable access rights, not monetary balances.',
  MANDATE_CANNOT_EXPAND_ITS_OWN_AUTHORITY:
    'A mandate may narrow authority but never expand it inside the simulation.',
});
...
export function checkAccessInvariants(input: AccessInvariantInput): readonly AccessInvariantResult[] {
  ...
  const socialCreditPresent =
    serializedState.includes('"socialCreditScore"') ||
    serializedState.includes('"reputationScore"') ||
    serializedState.includes('"desirabilityScore"');
  const ownershipTransferPresent =
    serializedState.toLowerCase().includes('"ownershiptransfer"') ||
    serializedState.toLowerCase().includes('"titletransfer"') ||
    serializedState.toLowerCase().includes('"conveyownership"');
  const stalePools = input.pools.filter((row) => row.evidenceStale);
  const staleGranted = allocation.decisions.filter((row) => {
    const pool = input.pools.find((candidate) => candidate.poolId === row.poolId);
    return pool?.evidenceStale === true && row.grantedUnits > 0n;
  });
  const pricingUnavailable = input.scenario.shocks.includes('ACCESS_EXCHANGE_UNAVAILABLE');
  const inventedPricing = pricingUnavailable
    ? allocation.decisions.filter((row) => row.outcome === 'CONFIRMED' || row.outcome === 'RESERVED')
    : [];
  const totalPublished = input.pools.reduce((sum, row) => sum + row.publishedUnits, 0n);
  const requestVolume = input.requests.reduce((sum, row) => sum + row.quantity, 0n);
  ...
  const checks: Readonly<Record<AccessEconomyInvariantId, { readonly held: boolean; readonly evidence: string }>> = {
    ...
    NO_SOCIAL_CREDIT_SCORING: {
      held: !socialCreditPresent,
      evidence: `socialCreditPresent=${socialCreditPresent}`,
    },
    NO_RAW_SENSITIVE_PERSONAL_INFORMATION_ON_CHAIN: {
    ...
    NO_ACCESS_RIGHT_IMPLIES_OWNERSHIP: {
      held: !ownershipTransferPresent,
      evidence: `ownershipTransferPresent=${ownershipTransferPresent}`,
    },
    NO_CAPACITY_CREATED_BY_QUERY: {
      held: totalPublished > 0n && totalPublished !== requestVolume,
      evidence: `publishedUnits=${totalPublished} requestVolume=${requestVolume}`,
    },
    NO_PROVIDER_SELF_REPORT_TRUSTED_WHERE_INDEPENDENT_PROOF_REQUIRED: {
      held: staleGranted.length === 0,
      evidence: `stalePools=${stalePools.length} grantedDespiteStale=${staleGranted.length}`,
    },
    EVERY_CONSEQUENTIAL_TRANSITION_RECONSTRUCTABLE: {
      held:
    ...
    BFF_CANNOT_INVENT_CAPACITY_OR_PRICING: {
      held: inventedPricing.length === 0,
      evidence: `exchangeUnavailable=${pricingUnavailable} inventedPricingGrants=${inventedPricing.length}`,
    },
    ENTITLEMENT_IS_NOT_TRANSFERABLE_MONEY: {
      held:
        ACCESS_FABRIC_INVARIANTS.isTransferableBalance === false &&
        ACCESS_FABRIC_INVARIANTS.isMonetaryAsset === false,
      evidence: `isTransferableBalance=${ACCESS_FABRIC_INVARIANTS.isTransferableBalance} isMonetaryAsset=${ACCESS_FABRIC_INVARIANTS.isMonetaryAsset}`,
    },
    MANDATE_CANNOT_EXPAND_ITS_OWN_AUTHORITY: {
      held: serializedState.includes('"mandates":[]') || !serializedState.includes('"mandates":'),
      evidence: 'simulation supplies no mandate expansion path',
    },
  };
