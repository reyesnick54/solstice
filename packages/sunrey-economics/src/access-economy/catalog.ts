...
export const ACCESS_ECONOMY_CATALOG: readonly AccessEconomyScenario[] = Object.freeze([
  ...
  scenario(
    ...
    'GPU-hour compute and metered AI capacity access',
    9116,
    'compute-abundance',
    ['ACCESS_ABUNDANCE_EXPANSION'],
    'AGGREGATE',
    'ABUNDANT',
    [
      pool('gpu-hour-pool', 'AI_COMPUTE', 'gpu_second', ['US-CA.bay-area'], ['2031-05-10'], ['provider.compute.a'], 3_500n),
    ],
    demand({ subjectCount: 80, requestsPerSubject: 4, meanQuantity: 120n, quantityJitter: 40n }),
    ['CONFIRMED'],
    'Compute access is metered usage capacity, not minted currency.',
  ),
  scenario(
    'ACCESS-SIM-17-robot-capacity',
    'robot-hours and automated-service capacity access',
    9117,
    'rapid-automation',
    ['ACCESS_PROVIDER_OUTAGE'],
    'PROVIDER',
    ...
    [
      pool(
        ...
        'robot_hour',
        ['DE-BY.munich', 'DE-NW.cologne'],
        ['2031-05-10'],
        ['provider.robot.a', 'provider.robot.b'],
        1_800n,
      ),
    ],
    demand({ subjectCount: 90, requestsPerSubject: 2, meanQuantity: 8n }),
    ['CONFIRMED', 'REFUSED_PROVIDER_UNAVAILABLE'],
    'Robot capacity refuses when a provider fails; no ownership transfer.',
  ),
  scenario(
    'ACCESS-SIM-18-energy-access',
    'kWh capacity, delivery and verified usage access',
    9118,
    'energy-scarcity',
    ['ACCESS_PRODUCTIVE_CAPACITY_FALL'],
    'AGGREGATE',
    'SCARCE',
    [
      pool('grid-kwh-pool', 'ENERGY', 'kWh', ['US-TX.dallas'], ['2031-05-10', '2031-05-11'], ['provider.energy.a'], 2_500n),
    ],
    demand({ subjectCount: 120, requestsPerSubject: 2, meanQuantity: 250n, quantityJitter: 80n, hotspotConcentrationBps: 9_000n }),
    ['CONFIRMED', 'REFUSED_CAPACITY_EXHAUSTED'],
    'Energy access follows verified delivery capacity; scarcity tightens under productive shock.',
  ),
]);
...
export function accessCatalogComplete(): boolean {
  return (
    ACCESS_ECONOMY_CATALOG.length === 18 &&
