...
describe('Chunk 89 post-genesis stabilization', () => {
  ...
  it('rejects the mandatory negative activation cases', () => {
    ...
    const exchangeWithoutEvidence = suite.exchangeWithoutEvidence;
    const custodyWithoutHsm = suite.custodyWithoutHsm;
    const fiatWithoutBanking = suite.fiatWithoutBanking;
    const himWithoutPrivacy = suite.himWithoutPrivacy;
    const wrongNetwork = suite.wrongNetwork;
    const replayedPackage = suite.replayedPackage;
    const aiActivation = suite.aiActivation;
    const restrictionBypass = suite.restrictionBypass;
    const genesisLeavesDisabled = suite.genesisLeavesDisabled;
    assert.ok(exchangeWithoutEvidence);
    assert.ok(custodyWithoutHsm);
    assert.ok(fiatWithoutBanking);
    assert.ok(himWithoutPrivacy);
    assert.ok(wrongNetwork);
    assert.ok(replayedPackage);
    assert.ok(aiActivation);
    assert.ok(restrictionBypass);
    assert.ok(genesisLeavesDisabled);
    assert.equal(exchangeWithoutEvidence.outcome, 'REJECTED');
    assert.ok(exchangeWithoutEvidence.reasons.includes('Exchange activation without required evidence rejected'));
    assert.equal(custodyWithoutHsm.outcome, 'REJECTED');
    assert.ok(custodyWithoutHsm.reasons.includes('custody activation without HSM evidence rejected'));
    assert.equal(fiatWithoutBanking.outcome, 'REJECTED');
    assert.ok(fiatWithoutBanking.reasons.includes('fiat activation without banking evidence rejected'));
    assert.equal(himWithoutPrivacy.outcome, 'REJECTED');
    assert.ok(himWithoutPrivacy.reasons.includes('Human Information market without privacy/legal evidence rejected'));
    assert.equal(wrongNetwork.outcome, 'REJECTED');
    assert.ok(wrongNetwork.reasons.includes('wrong-network package rejected'));
    assert.equal(replayedPackage.outcome, 'REJECTED');
    assert.ok(replayedPackage.reasons.includes('replayed package rejected'));
    assert.equal(aiActivation.outcome, 'REJECTED');
    assert.ok(aiActivation.reasons.includes('AI activation rejected'));
    assert.equal(restrictionBypass.outcome, 'REJECTED');
    assert.ok(restrictionBypass.reasons.includes('restriction bypass rejected'));
    assert.equal(restrictionBypassRejected('REWRITE_FINALIZED_BLOCKS'), true);
    assert.equal(genesisLeavesDisabled.outcome, 'REJECTED');
  });
