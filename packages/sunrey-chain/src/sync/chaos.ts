...
export function runChaosRecoverySuite(): SyncResult<ChaosRecoveryReport> {
  ...
  return opsOk({
    ...
    tamperedSnapshotRejected: verified.ok && verified.value.ok && tampered.ok && !tampered.value.ok,
    wrongNetworkRejected: verified.ok && verified.value.ok && wrongNet.ok && !wrongNet.value.ok,
