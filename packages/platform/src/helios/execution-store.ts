...
export type HeliosStoreSnapshot = {
  readonly workOrders: readonly HeliosExecutionWorkOrder[];
...
export class InMemoryHeliosWorkStore {
  private readonly workOrders = new Map<string, HeliosExecutionWorkOrder>();
  ...
  putWorkOrder(order: HeliosExecutionWorkOrder): HeliosExecutionWorkOrder {
    this.workOrders.set(order.workOrderId, order);
    return order;
  }

  getWorkOrder(workOrderId: string, customerId?: string): HeliosExecutionWorkOrder | undefined {
  ...
  reserveBudgetAtomic(input: {
    ...
    readonly now: UtcInstant;
  }): { readonly reservation: ResearchBudgetReservation; readonly workOrder: HeliosExecutionWorkOrder } | HeliosFailure {
  ...
  reconcileReservation(input: {
    ...
    readonly now: UtcInstant;
    readonly customerId: string;
  }): HeliosExecutionWorkOrder | HeliosFailure {
