...
export type GrowthAccessFailure = {
  readonly code:
...
): Result<VerifiedActorContext, GrowthAccessFailure> {
  if (!isVerifiedActorContext(actor)) {
    return err({
      code: 'ACTOR_CONTEXT_REQUIRED',
      message: 'Growth Orchestrator requires a verified ActorContext',
    });
  }
...
export function authorizeViewGrowthPlan(
  actor: unknown,
  subjectId: string,
): Result<VerifiedActorContext, GrowthAccessFailure> {
  return authorize(actor, subjectId, 'VIEW_GROWTH_PLAN');
}

export function authorizeConfirmMandate(
  actor: unknown,
  subjectId: string,
): Result<VerifiedActorContext, GrowthAccessFailure> {
  return authorize(actor, subjectId, 'CONFIRM_ECONOMIC_MANDATE');
}

export function authorizeOperateGrowth(
  actor: unknown,
  subjectId: string,
): Result<VerifiedActorContext, GrowthAccessFailure> {
  return authorize(actor, subjectId, 'OPERATE_GROWTH_ORCHESTRATOR');
}
