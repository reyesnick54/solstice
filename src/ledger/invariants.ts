...
export function assertIdempotencyKey(key: string): void {
  if (typeof key !== "string" || key.trim().length === 0) {
    throw new LedgerInvariantError(
      "IDEMPOTENCY",
      "an idempotency key is required on every journal",
    );
  }
}
...
export function assertNoFloatAmounts(postings: readonly ProposedPosting[]): void {
  for (const posting of postings) {
    if (!(posting.amount instanceof Money)) {
      throw new LedgerInvariantError(
        "BALANCE",
        "every posting amount must be a Money instance",
      );
    }
 ...
 */
export function assertNoCommingling(accounts: readonly Account[]): void {
  const classes = new Set(accounts.map((a) => a.class));
  if (classes.has("CUSTOMER") && classes.has("CORPORATE")) {
    throw new LedgerInvariantError(
      "NO_COMMINGLING",
...
export function assertClassBridge(
  ...
  const pair = new Set<AccountClass>([bridge.fromClass, bridge.toClass]);
