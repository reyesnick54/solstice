...
describe('architecture constitution', () => {
  ...
  it('registers the Phase B Platform API at the reserved services/api path', () => {
    ...
    assert.equal(evaluateCapability(manifest, 'sunrey-consumer-bff').status, 'IMPLEMENTED');
    assert.equal(evaluateCapability(manifest, 'sunrey-consumer-bff').owner, 'services/api');
    assert.equal(evaluateCapability(manifest, 'sunrey-consumer-platform').status, 'IMPLEMENTED');
    assert.equal(evaluateCapability(manifest, 'sunrey-consumer-platform').owner, 'packages/sunrey-sdk');
    ...
    assert.equal(existsSync(join(REPO_ROOT, 'services/api/src/consumer/orchestrator.ts')), true);
    assert.equal(existsSync(join(REPO_ROOT, 'services/consumer-platform/src/runtime.ts')), true);
