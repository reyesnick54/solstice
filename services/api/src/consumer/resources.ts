...
function descriptorFor(group: ConsumerResourceGroup): ConsumerResourceDescriptor {
  switch (group) {
    ...
    case 'GROW':
      return row(group, '/api/v1/grow', ['GET', 'POST', 'PATCH'], 'AVAILABLE_SIMULATION', 'packages/platform Growth Orchestrator + packages/personal-economic-graph', 'none', 'Opportunity feed plus PEG profile, snapshot, goals, insights, and suitability. Starting a proposal does not move money. Not the Ledger. Not guaranteed returns.');
    case 'GOALS':
