...
import { omitUndefined, pageSizeOf } from './pagination.ts';
...
export function dispatchAccess(
  ...
  if (pathWithoutQuery === '/api/v1/access/history' && method === 'GET') {
    return result(
      mapAccessOutcome(
        consumer.history(actor, omitUndefined({
  ...
  if (pathWithoutQuery.startsWith('/api/v1/access/transactions/') && pathWithoutQuery.endsWith('/confirm') && method === 'POST') {
    ...
    return result(
      mapAccessOutcome(
        consumer.confirmTransaction(actor, transactionId, omitUndefined({
          userApproved: rec.userApproved === true ? true : undefined,
          paymentMethodId: str(rec.paymentMethodId),
          idempotencyKey,
        })),
        requestId,
      ),
  ...
  if ((pathWithoutQuery === '/api/v1/access/quote' || pathWithoutQuery === '/api/v1/access/quotes') && method === 'POST') {
    if (str(rec.opportunityId)) {
      return result(
        mapAccessOutcome(
          consumer.createCheckoutQuote(actor, omitUndefined({
            opportunityId: str(rec.opportunityId)!,
            requestedUnits: num(rec.requestedUnits) ?? num(rec.quantity) ?? 1,
            start: str(rec.start) ?? str(rec.startsAt),
            end: str(rec.end) ?? str(rec.endsAt),
            selectedOptions:
              rec.selectedOptions && typeof rec.selectedOptions === 'object'
                ? (rec.selectedOptions as Record<string, string>)
                : undefined,
            idempotencyKey,
          })),
          requestId,
  ...
  if ((pathWithoutQuery === '/api/v1/access/reserve' || pathWithoutQuery === '/api/v1/access/checkout') && method === 'POST') {
    return result(
      mapAccessOutcome(
        consumer.reserve(actor, omitUndefined({
  ...
  if (pathWithoutQuery === '/api/v1/access/allocation/preview' && method === 'POST') {
    ...
    return result(
      mapAccessOutcome(
        product.accessAllocationPreview(actor, omitUndefined({
          epochId: str(rec.epochId),
          categories,
        })),
        requestId,
      ),
  ...
  if (Object.keys(query).length > 0) {
      return result(
        mapAccessOutcome(
          consumer.listEntitlements(actor, omitUndefined({
            category: categoryOf(query.category ?? ''),
            status: str(query.status),
            period: str(query.period),
            expiringSoon: bool(query.expiringSoon),
          })),
          requestId,
        ),
        headers,
  ...
  if (path.startsWith('/api/v1/access/transactions/') && path.endsWith('/cancel') && method === 'POST') {
    ...
    return result(
      mapAccessOutcome(
        product.cancelTransaction(actor, decodeURIComponent(id), omitUndefined({
          penaltyMinorUnits: str(rec.penaltyMinorUnits),
          providerRefundMinorUnits: str(rec.providerRefundMinorUnits),
        })),
        requestId,
      ),
      headers,
  ...
  if (path === '/api/v1/access/intents' && method === 'POST') {
    return result(
      mapAccessOutcome(
        product.createIntent(actor, omitUndefined({
  ...
  if (path === '/api/v1/access/availability' && method === 'POST') {
    return result(
      mapAccessOutcome(
        product.checkAvailability(actor, omitUndefined({
          category: categoryOf(rec.category) ?? 'MOBILITY',
          summary: str(rec.summary) ?? str(rec.description),
          location: str(rec.location),
          intentId: str(rec.intentId),
        })),
        requestId,
      ),
      headers,
  ...
  if (pathWithoutQuery === '/api/v1/access/search' && method === 'POST') {
    return result(
      mapAccessOutcome(
        consumer.search(actor, omitUndefined({
          category: categoryOf(rec.category) ?? 'MOBILITY',
          query: str(rec.query) ?? str(rec.summary) ?? '',
          location: str(rec.location),
          startDate: str(rec.startDate) ?? str(rec.start),
          endDate: str(rec.endDate) ?? str(rec.end),
          units: num(rec.units),
          unit: str(rec.unit),
          cursor: str(rec.cursor),
          pageSize: pageSizeOf(str(rec.pageSize) ?? str(rec.page_size)),
          filters: rec.filters && typeof rec.filters === 'object' ? (rec.filters as Record<string, string>) : undefined,
          sort: str(rec.sort),
        })),
        requestId,
      ),
  ...
  if (path === '/api/v1/access/quotes' && method === 'POST') {
    if (str(rec.providerId) && str(rec.catalogItemId)) {
      return result(
        mapAccessOutcome(
          product.createProviderQuote(actor, omitUndefined({
    ...
    return result(
      mapAccessOutcome(
        product.createQuote(actor, omitUndefined({
          category: categoryOf(rec.category) ?? 'MOBILITY',
          summary: str(rec.summary) ?? str(rec.description) ?? '',
          location: str(rec.location),
          intentId: str(rec.intentId),
  ...
  if (path === '/api/v1/access/reservations' && method === 'POST') {
    return result(
      mapAccessOutcome(
        product.createReservation(actor, omitUndefined({
          quoteId: str(rec.quoteId) ?? '',
          startsAt: str(rec.startsAt),
          endsAt: str(rec.endsAt),
          idempotencyKey,
        })),
        requestId,
      ),
  ...
  if (path === '/api/v1/access/redemptions/preview' && method === 'POST') {
    return result(
      mapAccessOutcome(
        product.previewRedemption(actor, omitUndefined({
          category: categoryOf(rec.category) ?? 'MOBILITY',
          ...
          redemptionId: str(rec.redemptionId),
          intentId: str(rec.intentId),
  ...
  if (path === '/api/v1/access/redemptions' && method === 'POST') {
    return result(
      mapAccessOutcome(
        product.startRedemption(actor, omitUndefined({
          category: categoryOf(rec.category) ?? 'MOBILITY',
          providerId: str(rec.providerId) ?? 'turo',
          quoteId: str(rec.quoteId) ?? '',
          entitlementId: str(rec.entitlementId) ?? '',
  ...
  if (path.startsWith('/api/v1/access/redemptions/') && path.endsWith('/confirm') && method === 'POST') {
    ...
    return result(
      mapAccessOutcome(
        product.confirmRedemption(actor, decodeURIComponent(id), omitUndefined({
          userApproved: rec.userApproved === true ? true : undefined,
          userFiatMinorUnits: str(rec.userFiatMinorUnits),
        })),
        requestId,
      ),
      headers,
  ...
  if (path === '/api/v1/access/experiences/quote' && method === 'POST') {
    return result(
      mapAccessOutcome(
        product.quoteExperience(actor, omitUndefined({
