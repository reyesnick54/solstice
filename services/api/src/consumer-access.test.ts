...
describe('Consumer BFF Human Access Economy', () => {
  it('requires authentication on access routes', () => {
    ...
    const res = call(world, 'GET', '/api/v1/access/overview', null);
  ...
  it('returns disabled overview for pending verification', () => {
    const world = createSandboxWorld();
    const res = call(world, 'GET', '/api/v1/access/overview', 'kyc_pending');
  ...
  it('exposes categories as presentation metadata', () => {
    const world = createSandboxWorld();
    const res = call(world, 'GET', '/api/v1/access/categories', 'basic_verified');
    assert.equal(res.status, 200);
  ...
  it('lists a food-access entitlement for the verified sandbox persona', () => {
    const world = createSandboxWorld();
    const res = call(world, 'GET', '/api/v1/access/entitlements', 'basic_verified');
    assert.equal(res.status, 200);
  ...
  it('supports Mustang in Miami reservation flow without fabricating unmatched quotes', () => {
    const world = createSandboxWorld();
    const intent = call(world, 'POST', '/api/v1/access/intents', 'basic_verified', {
      category: 'MOBILITY',
    ...
    const availability = call(world, 'POST', '/api/v1/access/availability', 'basic_verified', {
      category: 'MOBILITY',
      summary: 'Ford Mustang — Miami weekend',
      location: 'Miami, FL',
    ...
    const rejectedQuote = call(world, 'POST', '/api/v1/access/quotes', 'basic_verified', {
      category: 'MOBILITY',
  ...
  it('quotes and confirms a Japan 14-day experience', () => {
    const world = createSandboxWorld();
    const quoted = call(world, 'POST', '/api/v1/access/experiences/quote', 'basic_verified', {
      destination: 'Japan',
      durationDays: 14,
    ...
    const confirmed = call(world, 'POST', `/api/v1/access/experiences/${experienceId}/confirm`, 'basic_verified', {});
  ...
  it('includes access capability in /me/capabilities', () => {
    const world = createSandboxWorld();
    const res = call(world, 'GET', '/api/v1/me/capabilities', 'basic_verified');
    assert.equal(res.status, 200);
