...
import { createInternalOperationsRoutes } from './operations/routes.ts';
...
export function createRoutes(deps: RouteDependencies): readonly RouteDefinition[] {
    ...
    },
    {
      ...
      handler: async () => ({
        ...
        body: {
          ...
          releaseId: process.env.SUNREY_RELEASE_ID ?? null,
          sourceCommit: process.env.SUNREY_SOURCE_COMMIT ?? null,
          schemaMigrationHead: process.env.SUNREY_SCHEMA_MIGRATION_HEAD ?? null,
  ...
  routes.push(
    ...createInternalOperationsRoutes({
      config: deps.config,
      readiness: deps.readiness,
      operatorToken: deps.internalOperatorToken,
      persistenceConfigured: deps.config.databaseConfigured,
    }),
  );
