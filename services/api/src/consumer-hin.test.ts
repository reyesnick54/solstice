...
function marketplacePrincipal(customerId: string, persona: 'basic_verified' | 'exchange'): BffPrincipal {
...
function marketplaceRuntime(): ConsumerBffRuntime {
  const basic = marketplacePrincipal('cust_sandbox_basic', 'basic_verified');
  const other = marketplacePrincipal('cust_sandbox_exchange', 'exchange');
...
function callMarketplace(
  runtime: ConsumerBffRuntime,
  method: string,
  path: string,
  persona: 'basic_verified' | 'exchange',
) {
...
function contributionRuntime(): ConsumerBffRuntime {
  ...
  return {
    ...
    sessions,
    hinValue: createHinContributionSurface(),
  };
}
...
describe('Consumer BFF HIN rights marketplace', () => {
  it('lists subject-scoped information rights without ownership transfer', () => {
    ...
    const listed = callMarketplace(runtime, 'GET', '/api/v1/hin/rights', 'basic_verified');
    assert.equal(listed.status, 200);
    const body = listed.body as {
      items: { ownershipTransferred: boolean; usageRightOnly: boolean }[];
  ...
  it('shows active licenses and does not guarantee earnings', () => {
    const runtime = marketplaceRuntime();
    const licenses = callMarketplace(runtime, 'GET', '/api/v1/hin/licenses', 'basic_verified');
    assert.equal(licenses.status, 200);
    const earnings = callMarketplace(runtime, 'GET', '/api/v1/hin/earnings', 'basic_verified');
    ...
    const activity = callMarketplace(runtime, 'GET', '/api/v1/hin/earnings/activity', 'basic_verified');
    assert.equal(activity.status, 200);
  });

  it('denies cross-user rights and hides licensee controls', () => {
    const runtime = marketplaceRuntime();
    const other = callMarketplace(runtime, 'GET', '/api/v1/hin/rights', 'exchange');
    assert.equal(other.status, 200);
    const body = other.body as { items: unknown[] };
    assert.equal(body.items.length, 0);
    const licensee = callMarketplace(runtime, 'GET', '/api/v1/hin/licensee/credentials', 'basic_verified');
    assert.equal(licensee.status, 404);
  });

  it('pauses HIN participation', () => {
    const runtime = marketplaceRuntime();
    const paused = callMarketplace(runtime, 'POST', '/api/v1/hin/participation/pause', 'basic_verified');
    assert.equal(paused.status, 200);
    const participation = callMarketplace(runtime, 'GET', '/api/v1/hin/participation', 'basic_verified');
...
describe('Consumer BFF HIN contributions', () => {
  it('catalogs the read-only HIN resource and does not expose verification or issuance routes', () => {
    ...
    assert.ok(
      !CONSUMER_BFF_ROUTES.some(
        (row) => row.includes('hin') && (row.includes('verify') || row.includes('issuance') || row.includes('mint')),
      ),
    );
  });
  ...
  it('returns customer contributions and aggregate metrics without raw personal data', () => {
    ...
    const aggregate = metrics.body as {
      suppression: { individualRecordsExposed: boolean };
      economicValueInputs: { isMintAmount: boolean };
    };
