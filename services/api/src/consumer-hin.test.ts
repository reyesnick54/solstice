...
describe('Consumer BFF HIN contributions', () => {
  ...
  it('returns customer contributions and aggregate metrics without raw personal data', async () => {
    ...
    const list = await invokeConsumerBff(runtime, {
      method: 'GET',
    ...
    const metrics = await invokeConsumerBff(runtime, {
      method: 'GET',
    ...
    const summary = await invokeConsumerBff(runtime, {
      method: 'GET',
  ...
  it('rejects privileged verify and mint posts', async () => {
    const runtime = contributionRuntime();
    const verify = await invokeConsumerBff(runtime, {
      method: 'POST',
