...
describe('Consumer BFF Personal Data Vault', () => {
  it('serves vault home, categories, and records for the owner', () => {
    ...
    const categories = await call(world, 'GET', '/api/v1/data/vault/categories', 'vault_financial');
    assert.equal(categories.status, 200);
    const listed = await call(world, 'GET', '/api/v1/data/vault/records', 'vault_financial');
    ...
    const detail = await call(world, 'GET', `/api/v1/data/vault/records/${items[0]!.dataRecordId}`, 'vault_financial');
  ...
  it('isolates customers and does not expose another vault', () => {
    ...
    const financial = await call(world, 'GET', '/api/v1/data/vault/records', 'vault_financial');
    const foreignId = (financial.body as { items: { dataRecordId: string }[] }).items[0]?.dataRecordId;
    assert.ok(foreignId);
    const stolen = await call(world, 'GET', `/api/v1/data/vault/records/${foreignId}`, 'vault_minimal');
  ...
  it('corrects user-declared data and exports a portable bundle', () => {
    ...
    assert.ok(job);
    const patched = await call(world, 'PATCH', `/api/v1/data/vault/records/${job.dataRecordId}`, 'vault_employment', {
      reason: 'title update',
      payload: { employer: 'Northwind Labs', title: 'Senior Analyst', startedOn: '2024-03-01' },
    });
    assert.equal(patched.status, 200);
    const exported = await call(world, 'POST', '/api/v1/data/vault/export', 'vault_employment');
    assert.equal(exported.status, 202);
    const status = await call(world, 'GET', '/api/v1/data/vault/export/status', 'vault_employment');
    assert.equal(status.status, 200);
    const jobId = (exported.body as { exportId: string }).exportId;
    const bundle = await call(world, 'GET', `/api/v1/data/vault/export/${jobId}`, 'vault_employment');
  ...
  it('shows disputed and revoked sandbox personas', () => {
    ...
    const revoked = await call(world, 'GET', '/api/v1/data/vault/records', 'vault_revoked');
    assert.equal(revoked.status, 200);
    assert.equal((revoked.body as { items: unknown[] }).items.length, 0);
    const sources = await call(world, 'GET', '/api/v1/data/vault/sources', 'vault_multi_source');
