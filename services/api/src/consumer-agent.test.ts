...
function productizationRuntime(world: ReturnType<typeof createSandboxWorld>): ConsumerBffRuntime {
  return {
    bff: world.bff,
...
function runtimeRuntime(world: ReturnType<typeof createSandboxWorld>): ConsumerBffRuntime {
  return {
    bff: world.bff,
    sessions: world.sessions,
    identity: world.runtime.identity.service,
...
function callProductization(
  ...
  return handleConsumerBff(productizationRuntime(world), {
    method,
    path,
    query: {},
    body,
...
function callRuntime(
  ...
  return handleConsumerBff(runtimeRuntime(world), {
    method,
    path,
    query,
    body,
...
describe('Consumer BFF Agent productization', () => {
  it('opens a conversation, prepares a payment card, and refuses cross-user access', () => {
    ...
    const other = handleConsumerBff(productizationRuntime(world), {
      method: 'POST',
...
describe('Consumer BFF Agent runtime', () => {
  it('lists the sandbox agent and denies cross-user access', () => {
    const world = createSandboxWorld();
    const listed = callRuntime(world, 'GET', '/api/v1/agents', 'agent_enabled');
    ...
    const other = callRuntime(world, 'GET', `/api/v1/agents/${items[0]?.agentId}`, 'basic_verified');
    assert.equal(other.status, 403);
    const pausedByOther = callRuntime(world, 'POST', `/api/v1/agents/${items[0]?.agentId}/pause`, 'basic_verified');
  ...
  it('creates a conversation, streams a message, and keeps financial state unchanged', () => {
    ...
    const created = callRuntime(world, 'POST', `/api/v1/agents/${agentId}/conversations`, 'agent_enabled', {
      title: 'Home chat',
    });
    assert.equal(created.status, 201);
    const conversationId = (created.body as { conversationId: string }).conversationId;
    const posted = callRuntime(
      world,
      'POST',
  ...
  it('supports memory controls and pause', () => {
    const world = createSandboxWorld();
    const agentId =
      (callRuntime(world, 'GET', '/api/v1/agents', 'agent_enabled').body as { items: { agentId: string }[] }).items[0]
        ?.agentId ?? '';
    const memory = callRuntime(world, 'POST', `/api/v1/agents/${agentId}/memories`, 'agent_enabled', {
      category: 'USER_PREFERENCE',
      content: 'User prefers explanations in simple language.',
      source: 'USER_DECLARED',
    });
    assert.equal(memory.status, 201);
    const paused = callRuntime(world, 'POST', `/api/v1/agents/${agentId}/pause`, 'agent_enabled');
