...
describe('Consumer BFF native economy', () => {
  ...
  it('returns protocol-native SunRey and MoonRey supply without fabricating HIN metrics', async () => {
    const res = await invokeConsumerBff(economyRuntime(), {
      method: 'GET',
      path: '/api/v1/economy',
      query: {},
      body: {},
  ...
  it('rejects unknown assets and privileged POST issuance', async () => {
    const runtime = economyRuntime();
    const missing = await invokeConsumerBff(runtime, {
      method: 'GET',
    ...
    const mint = await invokeConsumerBff(runtime, {
      method: 'POST',
  ...
  it('returns verified productive-economy metrics without minting', async () => {
    const res = await invokeConsumerBff(economyRuntime(), {
      method: 'GET',
    ...
    const sources = await invokeConsumerBff(economyRuntime(), {
      method: 'GET',
