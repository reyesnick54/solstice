...
- `packages/sunrey-chain` — ACCESS-08 Access Fabric access rights, reservations, and commitments live at `src/access`. Capability `sunrey-access-rights-commitments`. Extends the existing chain owner and writes through `SunReyChainService`. OWNERSHIP is distinct from ACCESS, USAGE, LEASE, and RESERVATION; an access right never conveys title. No new blockchain, side ledger, token wrapper, Access Coin, minting pathway, or automatic SunRey/MoonRey issuance. Raw personal data, travel history, health data, personal preferences, private itinerary contents, and payment credentials stay off chain. Do not create `packages/access-chain`, `packages/access-ledger`, `packages/access-coin`, `packages/access-token`, `packages/reservation-chain`, `packages/rights-chain`, `packages/mobility-chain`, or `packages/entitlement-chain`.
- `packages/sunrey-chain` — Chunk 161 jurisdictional operating scope, licensing, regulatory-evidence binding, corridor eligibility, and product activation matrix live at `src/mainnet/operating-scope`. Extends mainnet readiness. Not legal advice. Does not issue Execution Authority. Capability `sunrey-production-operating-scope`. Do not create `packages/licensing`, `packages/global-regulation`, `packages/country-law`, or `packages/legal-engine`.
- `packages/sunrey-chain` — SunRey Chain simulation trust layer; not a second ledger. Operator and release paths: Chunk 54 `src/ops`, Chunk 156 `src/ops/control-room`, Chunk 63 `src/release-candidate`, Chunk 70 `src/launch-rehearsal`, Chunk 71 `src/economics`, Chunk 78 `src/release-candidate/economic`, Chunk 79 `src/governance-ops`, Chunk 84 `src/release-candidate/mainnet`, Chunk 87 `src/pregenesis`, Chunk 88 `src/genesis-execution`, Chunk 90 `src/production-handoff`, Chunk 91 `src/provider-runtime`, Chunk 92 `src/validator-operator`, Chunk 93 `src/public-data-plane`, Chunk 97 `src/wallet/mobile-sync`, Chunk 160 `src/mainnet/external-evidence`. Productive and oracle paths: Chunk 108/112 `src/economics/human-contribution-bridge`, Chunk 116 `src/productive/source-taxonomy` and `src/oracle/source-taxonomy`, Chunk 117 `src/productive/claim-candidate`, Chunk 118 `src/units`, Chunk 119 CanonicalProductiveMeasurement, Chunk 120/121 `src/productive/policy-governance/attribution`, Chunk 122 `src/productive/policy-governance/attribution-accounting`, Chunk 123/124 `src/productive/policy-governance/value-function`, Chunk 125 `src/productive/policy-governance/value-settlement` (GPUV is not MoonRey), Chunk 126 `src/productive/policy-governance/shadow-economics`, Chunk 127 `src/oracle/production`, Chunk 128 `src/oracle/production/certification`, Chunk 129 energy, Chunk 130 compute, Chunk 131 manufacturing, Chunk 132 logistics, Chunk 133 resources, Chunk 135 real-estate/infrastructure, Chunk 136 bandwidth, Chunk 137 goods/service-delivery, Chunk 138 `src/oracle/production/economic-data-fabric`. Not a second mint, unit authority, oracle consensus engine, productive registry, attribution engine, or economic asset registry. Production valuation remains inactive. New infrastructure feeds use `facility_hour`; historical `machine_h` is not reinterpreted. DATA_RATE is not DATA_VOLUME. Order, invoice, and payment are not productive output.
- `packages/sunrey-exchange` — canonical Exchange. Chunk 95 institutional market operations live at `src/ops`. Chunk 99 consumer portfolio, quote, and simple trading APIs live at `src/consumer`. Phase D external market-data provider contract lives at `src/market-data`. ACCESS-09 productive capacity markets and dual-economy clearing live at `src/access-fabric`; capability `sunrey-exchange-capacity-access`; fiat settles on the canonical Ledger, SunRey Coin and MoonRey Coin move only on canonical custody or native chain rails, and entitlement or reward credit is consumed at its owning port. No fixed SunRey/MoonRey ratio, no coin issuance, no third currency. Not a second ledger, matching engine, or `packages/market-data`. Do not create `packages/access-fabric`, `packages/capacity-exchange`, `packages/capacity-market`, `packages/dual-economy-clearing`, `packages/clearing-house`, `packages/entitlements`, or `packages/access-ledger`.
...
- `packages/sunrey-chain` — Phase H Productive Economy Data Platform lives at `src/productive/economy-data`. Extends productive, oracle, units, and GPUV owners. Observations are economic inputs. They do not mint MoonRey or set Exchange price. Do not create `packages/productive-economy-data` or `packages/moonrey-data-fabric`.
...
- `packages/sunrey-chain` — Chunk 163 governed production economic parameter authorization lives at `src/economics/production-activation/authorization`, orchestrated by `src/governance-ops`. Capability `sunrey-production-economic-authorization`. Does not choose tokenomics, activate production, or create a second mint. AUTHORIZED_CANDIDATE is not PRODUCTION_ACTIVE.
- `packages/sunrey-chain` — Chunk 164 immutable production launch candidate freeze lives at `src/release-candidate/mainnet/launch-freeze`. Capability `sunrey-production-launch-freeze`. Extends the existing mainnet release candidate. Freeze is not approval, authorization, or activation. Does not enable mainnet, flip LIVE_* flags, mint, or issue Execution Authority.
- `packages/sunrey-chain` — Chunk 165 frozen-candidate production ceremony, multi-party launch authorization, offline signing rehearsal, transcript integrity, and abort-safe human ceremony flow live at `src/production-ceremony/launch-candidate`. Extends Chunk 85 `sunrey-production-genesis-ceremony`. Binds the exact Chunk 164 freeze hash. Does not create a second genesis ceremony, generate real production keys, or activate production. LAUNCH_AUTHORIZATION_CANDIDATE is not MAINNET_ACTIVE.
- `packages/sunrey-chain` — Chunk 166 staged capability activation, domain-scoped canary rehearsal, progressive readiness gates, and independent product sequencing live at `src/post-genesis/staged-activation`. Extends `sunrey-post-genesis-stabilization`. Rehearsal only. Does not turn everything on, invent production limits, flip LIVE_* flags, or create `packages/activation`, `packages/canary`, `packages/mainnet-launch`, or `packages/product-switches`.
- `packages/sunrey-chain` — Chunk 167 launch abort, domain-scoped emergency restrictions, rollback semantics, recovery gates, and resumption authorization rehearsal lives at `src/governance-ops/launch-abort`, with recovery gates at `src/post-genesis` and application-rollback records at `src/production-handoff`. Extends `sunrey-governance-operations`, `sunrey-post-genesis-stabilization`, and `sunrey-production-handoff`. Composes with Chunk 165 ceremony abort and Chunk 166 pause and readiness gates. Does not create a second emergency authority, global kill switch, or super-admin. Application rollback is not chain-history rollback. Incident end does not auto-resume a capability. Do not create `packages/kill-switch`, `packages/emergency-admin`, `packages/rollback-engine`, `packages/incident-v2`, or `packages/recovery-v2`.
...
- `packages/custody` — provider-neutral simulation custody and Travel Rule. Chunk 153 dual-native-asset hardening and production-candidate provider framework live at `src/provider-candidate`. Phase D productized custody contract, deposit/withdrawal workflows, and webhook normalization extend that owner. Not a second custody or key authority.
- `packages/market-surveillance` — deterministic alerts and case proposals
- Chunk 152 regulated provider-candidate adapters live at `packages/identity/src/provider-candidate`, `packages/kernel/src/compliance/provider-candidate`, `packages/custody/src/provider-candidate`, and `packages/market-surveillance/src/provider-candidate`. Phase D blockchain analytics lives at `packages/kernel/src/compliance/provider-candidate/blockchain-analytics.ts`. Fixture adapters only. Not a second Kernel, KYC package, AML engine, Travel Rule network, or surveillance engine.
