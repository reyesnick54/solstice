...
- `packages/sunrey-exchange` — canonical Exchange. Chunk 95 institutional market operations live at `src/ops`. Chunk 99 consumer portfolio, quote, and simple trading APIs live at `src/consumer`. Phase D external market-data provider contract lives at `src/market-data`. Not a second ledger or `packages/market-data`.
...
- `packages/sunrey-economics` — Chunk 75 SunRey/MoonRey dual-economy simulation laboratory; not production monetary policy. ACCESS-13 Access Economy simulation, permanent access invariants, and end-to-end qualification live at `src/access-economy`. Extends the dual-economy and stress laboratories. Access activity issues no SunRey or MoonRey and asserts no fixed peg. `ACCESS_FABRIC_CODE_COMPLETE_CANDIDATE` is an engineering label; `PRODUCTION_READY`, `LIVE_CONNECTIVITY_ENABLED`, and `PRODUCTION_ACTIVE` stay false and are never derived from a passing test run. Do not create `packages/access-economy` or `packages/access-simulation`.
- `packages/access-fabric` — ACCESS-04 SunRey Access Fabric. Canonical access entitlement engine and Personal Access Envelope. Answers what a person may currently request; does not reserve, price, settle, or score. An `AccessEntitlement` is not money, not a transferable balance, and not a human-worth score, and `transferability` defaults to false. Not a second ledger, Exchange, custody system, or monetary authority. Do not create `packages/access-coin`, `packages/access-economy`, `packages/access-ledger`, `packages/access-exchange`, `packages/access-custody`, or `packages/entitlements`.
