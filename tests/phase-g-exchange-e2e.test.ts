...
describe('Phase G Exchange / wallet / coin E2E', () => {
  it('runs the sandbox BUY lifecycle through public Consumer BFF routes', async () => {
    const world = createPhaseGWorld();
    const home = await world.handle({ method: 'GET', path: '/api/v1/exchange' });
    ...
    const eligibility = await world.handle({ method: 'GET', path: '/api/v1/exchange/eligibility' });
    ...
    const ticker = await world.handle({ method: 'GET', path: `/api/v1/exchange/markets/${marketId}/ticker` });
    assert.equal(ticker.status, 200);
    const book = await world.handle({ method: 'GET', path: `/api/v1/exchange/markets/${marketId}/order-book` });
    assert.equal(book.status, 200);
    const holdings = await world.handle({ method: 'GET', path: '/api/v1/exchange/holdings' });
    ...
    const stepUp = await world.handle({
      method: 'POST',
    ...
    const after = await world.handle({ method: 'GET', path: '/api/v1/exchange/holdings' });
    assert.equal(after.status, 200);
  });

  it('repeats the SELL lifecycle with asset reservation and quote/asset balances', async () => {
    const world = createPhaseGWorld();
    ...
    const again = await world.handle({
      method: 'POST',
      path: '/api/v1/exchange/proposals',
  ...
  it('runs wallet deposit and withdrawal through public routes', async () => {
    const world = createPhaseGWorld();
    const wallet = await world.handle({ method: 'GET', path: '/api/v1/wallets' });
    assert.equal(wallet.status, 200);
    const address = await world.handle({ method: 'GET', path: '/api/v1/wallets/deposit-address' });
    assert.equal(address.status, 200);
    assert.ok((address.body as { address: string }).address);

    const deposit = await world.handle({
      method: 'POST',
    ...
    const withdrawn = await world.handle({
      method: 'POST',
      path: '/api/v1/wallets/withdrawals',
      body: { assetId: 'SUNREY_COIN', quantity: '1', destination: 'sr1ex_external', approved: true },
    });
    assert.equal(withdrawn.status, 200);
    const history = await world.handle({ method: 'GET', path: '/api/v1/wallets/transactions' });
    assert.equal(history.status, 200);
  });

  it('covers SunRey Coin and MoonRey Coin views without unauthorized issuance', async () => {
    const world = createPhaseGWorld();
    const sun = await world.handle({ method: 'GET', path: '/api/v1/economy/sunrey-coin' });
    assert.equal(sun.status, 200);
    assert.equal((sun.body as { unauthorizedIssuance: boolean }).unauthorizedIssuance, false);
    const moon = await world.handle({ method: 'GET', path: '/api/v1/economy/moonrey-coin' });
    ...
    const economy = await world.handle({ method: 'GET', path: '/api/v1/economy' });
    assert.equal(economy.status, 200);
    const status = await world.handle({ method: 'GET', path: '/api/v1/economy/status' });
  ...
  it('runs Agent Exchange proposal flow without self-approval', async () => {
    const world = createPhaseGWorld();
    const started = await world.handle({ method: 'POST', path: '/api/v1/agent/conversations', body: {} });
    assert.equal(started.status, 201);
    const conversationId = (started.body as { conversationId: string }).conversationId;
    const turn = await world.handle({
      method: 'POST',
